...
class SilenceWaiter:
    ...
    async def wait_for_silence(self) -> None:
        if not self.__bot_output_device:
            if self.__required:
                raise RuntimeError("BOT_OUTPUT_DEVICE must be set when running with --verify.")
            self.__logger.info("BOT_OUTPUT_DEVICE is not set. Skipping silence check.")
            return

        input_index = self.__find_device_index()
        device_info = sd.query_devices(input_index)
        self.__logger.info(f"Waiting for silence on device: {device_info['name']} (index {input_index})")
        frame_size = int(self.__sample_rate * self.__frame_duration)
        silent_frames_required = int(self.__required_silence / self.__frame_duration)
        silent_counter = 0
    ...
    def __find_device_index(self) -> Optional[int]:
        ...
        for idx, dev in enumerate(sd.query_devices()):
            if self.__bot_output_device.lower() in dev["name"].lower() and dev["max_output_channels"] > 0:
                return idx
        devices = [f"{idx}: {dev['name']}" for idx, dev in enumerate(sd.query_devices())]
        error_msg = (
